...
const upsertPriceRecord = async (
  ...
  if (upsertError?.message?.includes('foreign key constraint') && retryCount < maxRetries) {
    console.log(`Price insert failed due to missing product, retrying in 2s...`);
...
const createOrRetrieveCustomer = async ({
  ...
  if (!existingSupabaseCustomer && stripeIdToInsert) {
